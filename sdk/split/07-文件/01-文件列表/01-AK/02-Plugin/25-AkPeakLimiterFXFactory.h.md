...
AkPeakLimiterFXFactory.h 文件参考

[浏览源代码.](_ak_peak_limiter_f_x_factory_8h_source.html)

## 详细描述

Registers the Peak Limiter plugin automatically. This file should be included once in a .CPP (not a .h, really). The simple inclusion of this file and the linking of the library is enough to use the plugin. **WARNING**: Include this file only if you wish to link statically with the plugins. Dynamic Libaries (DLL, so, etc) are automatically detected and do not need this include file.   
**Wwise plugin name:** Peak Limiter   
**Library file:** AkPeakLimiterFX.lib

在文件 [AkPeakLimiterFXFactory.h](_ak_peak_limiter_f_x_factory_8h_source.html) 中定义.