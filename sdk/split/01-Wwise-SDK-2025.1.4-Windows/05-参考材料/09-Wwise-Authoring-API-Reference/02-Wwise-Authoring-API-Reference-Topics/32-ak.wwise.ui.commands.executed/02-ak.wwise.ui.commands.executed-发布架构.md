# ak.wwise.ui.commands.executed 发布架构
...
"description": "与所执行命令对应的对象。对象通常为选定对象。按照选项中指定的格式予以返回。若未使用任何对象，则可为空。",

"items": {

"type": "object",

"description": "对象。使用选项来指定所要返回的数据。默认返回 ID 和名称。\\n Wwise 对象。使用 return 选项来指定所需的属性。",
...
"description": "此数组中包含与所执行命令对应的平台。每个平台都表示为 ID (GUID)。"
