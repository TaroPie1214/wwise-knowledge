...
[选项架构](ak_wwise_ui_commands_executed_options_schema.html)

# 发布

| 名称 | 类型 | 说明 |
| --- | --- | --- |
| **command** \* | string | 所执行的命令 ID。 |
| **objects** \* | array | 与所执行命令对应的对象。对象通常为选定对象。按照选项中指定的格式予以返回。若未使用任何对象，则可为空。 |
| objects **[...]** | object | 对象。使用选项来指定所要返回的数据。默认返回 ID 和名称。  Wwise 对象。使用 return 选项来指定所需的属性。 |
...
| objects[...].regex(^((@?@?[a-zA-Z0-9:\_\(\),]+)|[a-zA-Z\(\),]+\(.+\))(\.@?@?[a-zA-Z0-9:\_\(\),]+|[a-zA-Z\(\),]+\(.+\))\*$)[...].**id** | string | 对象的 ID (GUID)。  以下形式的对象 GUID：{aabbcc00-1122-3344-5566-77889900aabb}。 |
| objects[...].regex(^((@?@?[a-zA-Z0-9:\_\(\),]+)|[a-zA-Z\(\),]+\(.+\))(\.@?@?[a-zA-Z0-9:\_\(\),]+|[a-zA-Z\(\),]+\(.+\))\*$)[...].**name** | string | 对象的名称。 |
| **platforms** \* | array | 此数组中包含与所执行命令对应的平台。每个平台都表示为 ID (GUID)。 |
| platforms **[...]** |  | Any of: |
| string | 对象的名称。 |
...
[发布架构](ak_wwise_ui_commands_executed_publish_schema.html)
