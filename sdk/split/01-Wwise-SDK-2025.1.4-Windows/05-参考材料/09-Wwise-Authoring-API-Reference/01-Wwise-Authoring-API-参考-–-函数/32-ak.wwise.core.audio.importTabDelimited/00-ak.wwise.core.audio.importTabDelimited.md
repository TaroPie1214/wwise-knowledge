...
| 名称 | 类型 | 说明 |
| --- | --- | --- |
| **importLocation** |  | 用作根相对对象路径的对象 ID (GUID)、名称或路径。  Any of: |
...
| **importLanguage** \* | string | 导入音频文件导入操作所用的语言（基于工程定义的语言，可在 WPROJ 文件的 LanguageList 中找到）。 |
...
| **importFile** \* | string | Tab Delimited Import 文件的位置。For using WAAPI on Mac, please see [在 Mac 上使用 WAAPI](waapi_prepare.html#waapi_path_on_mac) . |
...
[参数架构](ak_wwise_core_audio_importtabdelimited_args_schema.html)
...
[选项架构](ak_wwise_core_audio_importtabdelimited_options_schema.html)
...
| objects **[...]** | object | 在 Tab Delimited Import 流程中导入的对象。这些对象具有 return 选项中指定的属性（如适用）。  Wwise 对象。使用 return 选项来指定所需的属性。 |
...
[结果架构](ak_wwise_core_audio_importtabdelimited_result_schema.html)
