...
[Wwise 帮助文档](../../../00-Wwise-帮助文档.md) > [参考主题](../../00-参考主题.md) > [Profiler 视图](../00-Profiler-视图.md) > [Advanced Profiler](00-Advanced-Profiler.md) > Sends

### Sends

Advanced Profiler 中的 Sends 选项卡中，针对声音引擎当前播放的对象（每个声部或播放实例），显示了哪些发送目前处于活动状态。
...
| Auxiliary Bus | 为辅助发送设置的 Auxiliary Bus 名称。 |
| Send Volume | 发送音量，单位是分贝（dB）。 |
| Send Type | 使用的发送类型。可以是：  - Game-defined auxiliary send (游戏定义的辅助发送）（0 至 3） - 用户定义的辅助发送 (游戏定义的辅助发送）（0 至 3） |
