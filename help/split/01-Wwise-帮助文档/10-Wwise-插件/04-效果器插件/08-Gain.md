...
[Wwise 帮助文档](../../00-Wwise-帮助文档.md) > [Wwise 插件](../00-Wwise-插件.md) > [效果器插件](00-效果器插件.md) > Gain

## Gain

为了提升声音效果，您可能会想要去影响音频信号的幅度。

您可以通过 Gain（增益）插件来单独控制全频带声道和 LFE 声道的增益。 这些属性还可使用 RTPC 映射至游戏参数。
...
|  |  |
| --- | --- |
| Full Band Gain | 全频带增益。作用于除 LFE 之外的所有声道的增益。  Default value: 0  Range: -96.3 to 24  Units: dB |
| LFE Gain | LFE 增益。作用于 LFE 声道的增益（如果有 LFE 声道的话）。  Default value: 0  Range: -96.3 to 24  Units: dB |
